...
# Conectar a la base
db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "videogames.db")
...
with sqlite3.connect(db_path) as conn:
	...
	EU_sales = pd.read_sql("SELECT * FROM eu_sales_name", conn)
	NA_sales = pd.read_sql("SELECT * FROM na_sales_name", conn)
	EU_gre_sales = pd.read_sql("SELECT * FROM eu_sales", conn)
	NA_gre_sales = pd.read_sql("SELECT * FROM na_sales", conn)
...
render_bar_section("Top Juegos Más Vendidos en Europa", EU_sales, "Name", "EU_Sales")
render_bar_section("Top Juegos Más Vendidos en Norteamérica", NA_sales, "Name", "NA_Sales")
render_bar_section("Ventas por Género en Europa", EU_gre_sales, "Genre", "EU_Sales")
render_bar_section("Ventas por Género en Norteamérica", NA_gre_sales, "Genre", "NA_Sales")